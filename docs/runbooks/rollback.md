...
1. Set the Terraform traffic weight to the last safe value (zero for
   integrity or security failures), review the DNS-only plan, and apply it.
2. Stop writers and workers if they can corrupt or duplicate work;
...
4. Reconcile writes made during partial traffic. Resolve conflicts by the
   ...
   timestamp alone.
5. Verify application health, tenant isolation, queues, streams, reports, and alerts.
