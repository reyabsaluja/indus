...
async function fetchStockData(symbol: string): Promise<StockData | null> {
  try {
    ...
    console.log(`Data received for ${symbol}:`, result.data);
    ...
    console.log(`Processed data for ${symbol}:`, stockData);
    return stockData;
  } catch (error) {
              ...
              <path
                d={chartData
                  .map((pricePoint, i) => {
                    ...
                    const y =
                      35 -
                      ((pricePoint - minPrice) / (maxPrice - minPrice)) * 30;
                    return `${i === 0 ? "M" : "L"} ${x} ${y}`;
                  })
                  .join(" ")}
                fill="none"
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-muted-foreground max-w-sm">
              Add stocks to your favorites by clicking the star icon on any
              company page. They&apos;ll appear here for quick access.
            </p>
          </CardContent>
        </Card>
      </div>
